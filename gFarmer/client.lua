...
AddEventHandler("ZavrsiPoslaoMrtvi", function()
    ...
    if poceo == true and kamionPun == true then
        ...
        if kamionPun == true then
            exports['mythic_notify']:DoHudText('inform', '"Dovezao si kamion sa jabukama </span> izvoli svoju isplatu za transport"')
            kamionPun = false
...
AddEventHandler("uzmiKamiocic", function()
    ...
    if GlobalState.Uzeo >= 25 and poceo == true and kamionPun == true then
        ...
        exports['mythic_notify']:DoHudText('inform', '"Uzeo si kamion vozi do oznacene lokacije"')
        ...
        exports['mythic_notify']:DoHudText('inform', '"Nema dovoljno jabuka za izvoz"')
    end
end)
...
AddEventHandler("ajmoooo", function()
    ...
    if poceo == true and uzeoAuto == true then
        ...
        if uzeoAuto == true then
            vratiPareCoveku()
            exports['mythic_notify']:DoHudText('inform', '"Dobio si novac za vračanje vozila"')
            uzeoAuto = false
...
ESX.TriggerServerCallback('proveriPare', function(imaPare)
    if imaPare and poceo == true then
        ...
        TaskWarpPedIntoVehicle(ped, vehicle, -1)
        SetVehicleEngineOn(vehicle, true, true)
        exports['mythic_notify']:DoHudText('inform', '""Platio si za vađenje vozila""')
        Plate = GetVehicleNumberPlateText(vehicle)
        uzeoAuto = true
        ...
        exports['mythic_notify']:DoHudText('inform', '"Nemate dovoljno novca kod sebe ili se zaposlio"')
        end
    end)
end)
...
AddEventHandler("zavrsiPoslic", function()
    ...
    ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
        ...
        exports['mythic_notify']:DoHudText('inform', '"Završio si posao"')
    uzeoAuto = false
    kamionPun = false
    poceo = false
        uzeto = 0
            ...
            for k,v in ipairs(Config.Lokacije) do
                    if (GetDistanceBetweenCoords(pozicija, v.x, v.y, v.z, true) < 2) and not v.Ubrano then
                        ...
                        if IsControlJustPressed(0, Dugmici["E"]) then
                            ...
                            print(uzeto)
                            if uzeto == 25 then
                                exports['mythic_notify']:DoHudText('inform', '"Pokupili ste sve jabuke , idite do radnika da ostavite jabuke."')
                            v.Ubrano = false
                        end
                    end
                end
...
local blipovi = {
    {title="Prodavac Jabuka", colour=2, id=108, x = -1996.24, y = 591.12, z = 117.12},
}
